<div class="relative">
    ...
    @if (strlen($search) > $searchLengthBeforeStart)
        <div class="absolute z-50 bg-gray-800 text-xs rounded-lg w-64 mt-2 overflow-hidden">
            @if (count($results) > 0)
                <ul>
                    @foreach($results as $game)
                        <li class="border-b border-gray-700">
                            <a href="{{ url('/games/' . $game['slug']) }}" class="block hover:bg-gray-700 px-3 py-3 flex items-center transition ease-in-out duration-150">
                                @if (isset($game['cover']))    
